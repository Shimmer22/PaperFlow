...
- 可选 Provider：`deepseek`、`glm_api`、`nvidia`
- 主模型默认开启 thinking，sub 阶段默认关闭 thinking
- 默认模型：主 agent `deepseek-v4-pro`，sub agent `deepseek-v4-flash`
...
prompts/                 模型提示词
templates/               Typst 模板
providers/               Provider 配置（DeepSeek / GLM / NVIDIA）
scripts/                 启动与维护脚本
ui/                      Electron 前端
outputs/                 每次运行产物
tests/                   单元测试
...
- [deepseek.example.yaml](/Users/hui/Documents/Paper_Flow/providers/deepseek.example.yaml)
...
```bash
./scripts/run_deepseek.sh --idea "结合波束成形与自注意力"
```
...
- `research-flow provider-check --provider-config providers/deepseek.example.yaml`
- `research-flow ui`
...
### 1) DeepSeek / NVIDIA 返回 401

通常是 API key 缺失或无效。DeepSeek 请检查 `DS_API_KEY`，NVIDIA 请检查 `NV_API_KEY`。

### 2) NVIDIA 返回 410 Gone
